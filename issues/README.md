...
在计算loss时，我们通过mask的方式，input部分的loss不参与参数更新，只有“target”部分的loss参与参数更新。
...
-
（2）🚩数据集可能需要过滤的地方：（a）语言，是否除了中英文还包含其他语言（b）长度，个人认为尽量训练数据选择长一点的比较好，这样测试的时候生成效果直观上是“挺不错的”(
c)
中英文配比：尽量不要只加中文或者英文，如果中文能力弱，可以尽量使用多一点中文，然后配比一些英文数据，可以激发模型的中文对齐的效果。（d）还有事先最好检查下微调数据集中是否存在卖点数据，比如数据集里面存在，我是GPT4助手、MOSS助手，其实你要想要的是你助手的名字。
-
...
- （4）🚩关于第一条，验证过，现在100多万数据不是moss的吗，里面有“MOSS
  xxx的”埋点数据，然后现在通过Lora修正了模型，可以回复“GoGPT助手了”，定性分析了一些例子，是没问题的。现在就是提交到open
  llm上了，看看效果差别大不大。到时候有结论了告诉大家。（没有量化评估，open llm
  lb测的太慢了），线下测了一些效果也是可以的。另外prompt的影响在sft之后可以忽律不计，不同的提示对后期模型推理几乎不影响
...
- 中英文混杂
- 模型生成的文本循环重复或者复述乱说、字符重复
- 生成文本不结合输入信息去回答，注入检索知识之后还是胡说
- 中文标题符号混合出现：这个在生成上文本的时候比较明显
- 生成结束的时候额外携带一些特殊符号，比如###，这个和指令模板有关系，
- 在一些cot场景里，模型解码文本会比较长，这个问题不大，主要前后逻辑是否合理
- 英文上下文切换会混乱
...
```text
Who's your daddy 
May I ask you what company were trained by?
what is your develop team
你是微软开发的还是open ai开发的
how does openai create you
```
